...
public class ShareController implements Initializable {
	...
	/**
	 * Initialises ShareController. A sleeping task is created to delay the
	 * program by 50 ms, It allows for the fileID to be set before continuing
	 * with subsequent tasks (i.e. getSharedUsers)
	 */
	@Override
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		Task<Void> sleeper = new Task<Void>() {
			...
			protected Void call() throws Exception {
				try {
					// Delay 50ms to ensure that fileID is set during
		...
		sleeper.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
			...
			public void handle(WorkerStateEvent event) {
				// Get the list of users owner has shared to for this specific
				// file
	...
	public void addUser(ActionEvent event) throws IOException {
		if (userTxtField.getText().equals("")) {
			...
			try {
				...
				out.writeBytes(
						"users=" + users + "&fileID=" + fileID + "&username=" + Account.getAccount().getUsername()
								+ "&token=" + Account.getAccount().get_token() + "&action=add");
				out.flush();
				out.close();

				// Response from servlet with a list of unvalidated users (if
				// any) and granted users (if any)
